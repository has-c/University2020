...
prediction1.pred = predict(drinks.fit, newdata = pred1.df, type = "response")

#point estimates male and female of age 40
pred2.df = data.frame(age=c(40,40), sex=c("M", "F"))
prediction2.pred = predict(drinks.fit, newdata = pred2.df, type = "response")
...
highCholPredictions <- predict(heartattack.fit, newdata = pred.df, type="response")
#low cholestrol predictions 
pred.df <- data.frame(age=seq(0,100), chol="Low")
lowCholPredictions <- predict(heartattack.fit, newdata = pred.df, type="response")
